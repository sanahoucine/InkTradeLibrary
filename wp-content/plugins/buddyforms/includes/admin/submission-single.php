...
esc_html_e( 'Entry', 'buddyforms' );
?></span></h3>
				<div class="inside">
					<script>
						jQuery(document).ready(function () {
							jQuery("#metabox_<?php 
echo  esc_js( $form_slug ) ;
?> :input").attr("disabled", true);
							jQuery('#metabox_<?php 
echo  esc_js( $form_slug ) ;
?>').prop('readonly', true);
...
?>
		...
		<div id="postbox-container-1" class="buddyforms-metabox postbox-container">
			<div id="submitdiv" class="buddyforms-metabox postbox">
				...
				<h3 class="hndle"><span><?php 
esc_html_e( 'Entry Actions', 'buddyforms' );
?></span></h3>
				<div class="inside">
					<div class="submitbox">
...
?></a>
			...
			<div class="buddyforms-metabox postbox frm_with_icons">
				<h3 class="hndle"><span><?php 
esc_html_e( 'Entry Details', 'buddyforms' );
?></span></h3>
				<div class="inside">
					...
					<div class="misc-pub-section">
						...
						<?php 
esc_html_e( 'Entry ID', 'buddyforms' );
?>:
						<b><?php 
    ...
    if ( $bf_registration_user_id ) {
        ?>
					<div class="buddyforms-metabox postbox">
						<h3 class="hndle"><span><?php 
        esc_html_e( 'WordPress User', 'buddyforms' );
        ?></span></h3>
						<div class="inside">
							<div class="misc-pub-section">
								<p><?php 
        esc_html_e( 'User ID:', 'buddyforms' );
        ?> <?php 
        echo  esc_html( $bf_registration_user_id ) ;
        ?></p>
								<p>
									<span class="dashicons dashicons-admin-users"></span>&nbsp;
									<a href="<?php 
        echo  esc_attr( get_edit_user_link( $bf_registration_user_id ) ) ;
        ?>"><?php 
        esc_html_e( 'Edit User', 'buddyforms' );
        ?></a>
								</p>
							</div>
						</div>
...
if ( buddyforms_core_fs()->is_not_paying() && !buddyforms_core_fs()->is_trial() ) {
    ?>
				<div class="buddyforms-metabox postbox">
					<h3 class="hndle"><span><?php 
    esc_html_e( 'Get all insights about your user', 'buddyforms' );
    ?></span></h3>
					<div class="inside">
						<?php 
